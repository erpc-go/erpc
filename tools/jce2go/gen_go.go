...
// This file was generated by erpc2go ` + VERSION + `
...
`)
	...
	gen.code.WriteString("\"" + gen.codecPath + "/protocol/res/requestf\"\n")
	gen.code.WriteString("m \"" + gen.codecPath + "/model\"\n")
	gen.code.WriteString("\"" + gen.codecPath + "/protocol/codec\"\n")
	gen.code.WriteString("\"" + gen.codecPath + "/protocol/tup\"\n")
	gen.code.WriteString("\"" + gen.codecPath + "/protocol/res/basef\"\n")
	gen.code.WriteString("\"" + gen.codecPath + "/util/tools\"\n")
	gen.code.WriteString("\"" + gen.codecPath + "/util/current\"\n")
...
func (gen *GenGo) genIFProxyFun(interfName string, fun *FunInfo, withContext bool, isOneWay bool) {
  ...
  `)
	c.WriteString("buf := jce.NewBuffer()")
	var isOut bool
...
tarsResp := new(requestf.ResponsePacket)`)
	...
	if (isOut || fun.HasRet) && !isOneWay {
		c.WriteString("readBuf := jce.NewReader(tools.Int8ToByte(tarsResp.SBuffer))")
	}
...
func (gen *GenGo) genIFDispatch(itf *InterfaceInfo) {
  ...
  `)
	...
	if param {
		c.WriteString("readBuf := jce.NewReader(tools.Int8ToByte(tarsReq.SBuffer))")
	} else {
		c.WriteString("readBuf := jce.NewReader(nil)")
	}
	c.WriteString(`
	buf := jce.NewBuffer()
